NATIVE ANTIGRAVITY WORKFLOW: HYPER-RIGOROUS TEMPORAL & COGNITIVE GROUNDING ENGINE (VERSION: 0.0.1 & so on)
...
----------------------------------------------------------------------------------------------------
PHASE 9: GIT RAW-LOG COMMIT DISCIPLINE
----------------------------------------------------------------------------------------------------
9.1 The audit trail is incomplete unless the workspace is inside git and the finished change set is committed.
9.2 Stage the versioned raw conversation log artifact under `rigorous_testing\raw_conversation_log*.jsonl` with the source, report, plan, or workflow files changed in the same cycle.
9.3 Commit after every coherent change set. Do not batch unrelated harness, UI, simulation, and report edits into one opaque commit.
9.4 If the live raw transcript cannot be copied, record that failure explicitly in the audit report or commit message.
9.5 Do not commit secrets, local virtual environments, generated caches, downloaded model archives, `node_modules`, or bulky dataset binaries without explicit user instruction.
====================================================================================================
